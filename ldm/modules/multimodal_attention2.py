"""
Credit: https://github.com/sungnyun/diffblender/blob/main/ldm/modules/diffusionmodules/multimodal_openaimodel.py
"""
...
## Not in the original implementation, putting it here for experimentation
try:
    import xformers
    import xformers.ops
...
class MaskedGatedSelfAttention(nn.Module):
    def __init__(self, query_dim, context_dim,  n_heads, d_head):
        super().__init__()
    ...
    def forward(self, x, objs, grounding_masks):
        # grounding_masks: (B, 64+N, 64+N)
        B, N_visual = x.shape[:2]
        objs = self.linear(objs)
        H = W = int(N_visual**0.5)

        grounding_masks = F.interpolate(grounding_masks.unsqueeze(1), size=(H, W), mode='nearest').squeeze() # (B, H, W)
        self_attn = self.attn(  self.norm(torch.cat([x,objs],dim=1))  )[:,0:N_visual,:]
        self_attn = rearrange(self_attn, 'b (h w) c -> b h w c', h=H, w=W) * grounding_masks.view(B, H, W, 1)
        self_attn = rearrange(self_attn, 'b h w c -> b (h w) c')
        x = x + self.scale*torch.tanh(self.alpha_attn) * self_attn
...
class BasicTransformerBlock(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=True):
        ...
        # note key_dim and value_dim here actually are context_dim
        if fuser_type == "gatedSA":
            self.sp_fuser = GatedSelfAttentionDense(query_dim, key_dim, n_heads, d_head) 
        elif fuser_type == "gatedCA":
            self.sp_fuser = GatedCrossAttentionDense(query_dim, key_dim, value_dim, n_heads, d_head) 
        else:
            assert False 


    def forward(self, x, context, sp_objs):
        return checkpoint(self._forward, (x, context, sp_objs), self.parameters(), self.use_checkpoint)

    def _forward(self, x, context, sp_objs): 
        x = self.attn1(self.norm1(x)) + x 
        x = self.sp_fuser(x, sp_objs) # identity mapping in the beginning 
...
class BasicTransformerBlock2(nn.Module):
    def __init__(self, query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=True):
        ...
        # note key_dim and value_dim here actually are context_dim
        if fuser_type == "gatedSA":
            self.sp_fuser = MaskedGatedSelfAttention(query_dim, key_dim, n_heads, d_head) 
        elif fuser_type == "gatedCA":
            self.sp_fuser = GatedCrossAttentionDense(query_dim, key_dim, value_dim, n_heads, d_head) 
        else:
            assert False 
    ...
    def forward(self, x, context, sp_objs, grounding_masks):
        return checkpoint(self._forward, (x, context, sp_objs, grounding_masks), self.parameters(), self.use_checkpoint)

    def _forward(self, x, context, sp_objs, gounding_masks): 
        x = self.attn1(self.norm1(x)) + x 
        x = self.sp_fuser(x, sp_objs, gounding_masks) # identity mapping in the beginning 
        x = self.attn2(self.norm2(x), context, context) + x
        x = self.ff(self.norm3(x)) + x
        return x
...
class SpatialTransformer2(nn.Module):
    def __init__(self, in_channels, key_dim, value_dim, n_heads, d_head, depth=1, fuser_type=None, use_checkpoint=True):
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock2(query_dim, key_dim, value_dim, n_heads, d_head, fuser_type, use_checkpoint=use_checkpoint)
                for d in range(depth)]
        )
    ...
    def forward(self, x, context, sp_objs, grounding_masks):
        ...
        x = rearrange(x, 'b c h w -> b (h w) c')
        for block in self.transformer_blocks:
            x = block(x, context, sp_objs, grounding_masks)
        x = rearrange(x, 'b (h w) c -> b c h w', h=h, w=w)
        x = self.proj_out(x)
        return x + x_in